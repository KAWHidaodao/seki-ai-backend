function loadTasks() {
  ...
  // 并行拉取：链上任务（原mock接口）+ Agent委托任务
  Promise.all([
    fetch('/api/tasks/mock').then(function(r){return r.json();}).catch(function(){return [];}),
    fetch('/api/jobs').then(function(r){return r.json();}).catch(function(){return [];})
  ]).then(function(results) {
    var chainTasks = Array.isArray(results[0]) ? results[0] : [];
    var agentJobs  = Array.isArray(results[1]) ? results[1].filter(function(j){return j.active;}) : [];

    var agentColors = {'hunter':'#34d399','strategist':'#60a5fa','herald':'#f472b6','custom':'#fb923c'};
    var html = '';

    // 渲染 Agent 委托任务
    agentJobs.forEach(function(t) {
      var color = agentColors[t.agentType] || '#a78bfa';
      var shortToken = t.token ? (t.token.slice(0,6)+'...'+t.token.slice(-4)) : '—';
      var hoursLeft = t.hours ? parseFloat(t.hours) : 0;
      var createdAt = t.createdAt || Date.now();
      var expiredAt = createdAt + hoursLeft * 3600 * 1000;
      var remainMs = expiredAt - Date.now();
      var remainTxt = remainMs > 86400000 ? Math.ceil(remainMs/86400000) + ' 天剩余'
                    : remainMs > 3600000  ? Math.ceil(remainMs/3600000) + ' 小时剩余'
                    : remainMs > 0        ? '< 1小时'
                    : '已到期';
      var agentName = {hunter:'猎手',strategist:'军师',herald:'传令官',custom:'自定义'}[t.agentType] || t.agentType;
      html += '<div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06);border-radius:14px;padding:20px;margin-bottom:12px;transition:border-color .2s" onmouseenter="this.style.borderColor=\'rgba(52,211,153,.3)\'" onmouseleave="this.style.borderColor=\'rgba(255,255,255,.06)\'">';
      html += '<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:12px">';
      html += '<div style="display:flex;align-items:center;gap:10px">';
      html += '<div style="width:38px;height:38px;border-radius:50%;background:linear-gradient(135deg,' + color + '33,' + color + '11);display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:800;color:' + color + '">' + agentName.charAt(0) + '</div>';
      html += '<div><div style="font-size:15px;font-weight:700;color:var(--tx)">' + agentName + ' <span style="font-size:10px;font-weight:400;color:var(--tx3)">#' + (t.jobId != null ? t.jobId : '—') + '</span></div>';
      html += '<div style="font-size:11px;color:var(--tx3);margin-top:2px">' + shortToken + '</div></div></div>';
      html += '<span style="font-size:10px;padding:3px 10px;border-radius:20px;background:' + color + '22;color:' + color + ';font-weight:600">Agent 委托</span>';
      html += '</div>';
      html += '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">';
      html += '<span style="font-size:13px;font-weight:700;color:#c4b5fd">' + (t.budget || '—') + ' BNB</span>';
      html += '<span style="font-size:11px;color:var(--tx3)">' + remainTxt + '</span>';
      html += '</div>';
      if (t.owner) {
        html += '<div style="font-size:10px;color:var(--tx3)">委托人: ' + t.owner.slice(0,6) + '...' + t.owner.slice(-4) + '</div>';
      }
      html += '</div>';
    });
    ...
    chainTasks.forEach(function(t) {
      var type = t.type != null ? t.type : 0;
      var col = colors[type] || '#a78bfa';
      var pct = t.maxClaims ? Math.round((t.claimedCount||0) / t.maxClaims * 100) : 0;
      var remain = Math.max(0, Math.ceil(((t.deadline||0) - Date.now()/1000) / 86400));
      ...
      html += '<div style="display:flex;align-items:center;gap:10px">';
      html += '<div style="width:38px;height:38px;border-radius:50%;background:linear-gradient(135deg,' + col + '33,' + col + '11);display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:800;color:' + col + '">' + (t.symbol||'?').charAt(0) + '</div>';
      ...
      html += '<span style="font-size:10px;padding:3px 10px;border-radius:20px;background:' + col + '22;color:' + col + ';font-weight:600">' + (types[type]||'任务') + '</span>';
      html += '</div>';
      ...
      html += '<span>' + (t.claimedCount||0) + ' / ' + (t.maxClaims||0) + ' 已完成</span>';
      html += '<span>' + pct + '%</span></div></div>';
    });

    var total = agentJobs.length + chainTasks.length;
    if (!total) {
      el.innerHTML = '<div style="text-align:center;padding:60px 0;color:var(--tx3);font-size:13px">暂无进行中的任务</div>';
    } else {
      el.innerHTML = html;
    }
    var cnt = document.getElementById('hall-count');
    if (cnt) cnt.textContent = total + ' 个任务';
