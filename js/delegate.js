async function submitDelegate(){
 ...
 const jobRes = await fetch('https://seki-ai.com/api/jobs', {
   method:'POST', headers:{'Content-Type':'application/json'},
   body: JSON.stringify({token:tok, agentType:_dgSelectedAgent, budget:bnb, hours:mins, tg, jobId, owner:addr, thresholds})
 }).catch(err=>{
   console.error('[delegate] 通知后端失败:', err);
   toast('⚠️ Agent 激活失败，请联系客服（Job #'+jobId+'）', 'e');
   throw new Error('通知后端激活 Agent 失败: '+err.message);
 });
 if(!jobRes.ok){
   const errText = await jobRes.text().catch(()=>'');
   toast('⚠️ Agent 激活失败，请联系客服（Job #'+jobId+'）', 'e');
   throw new Error('后端返回错误: '+jobRes.status+' '+errText);
 }
